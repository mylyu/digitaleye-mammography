...
def build_feature_vector(
    ...
    Aggregate: support stats (count, IoU moments, score stats, mean area, anchor area, interactions)
    ...
    for m in model_names:
        ...
        if b is None or iou < min_neighbor_iou:
            ...
            support_ious.append(float(iou))
            support_scores.append(float(s))
            support_areas.append((bw * bh) / max(W * H, 1e-6))

    support_count = float(len(support_ious))
    denom_models = float(max(len(model_names), 1))
    sum_iou = float(sum(support_ious)) if support_ious else 0.0
    mean_iou = sum_iou / support_count if support_count > 0 else 0.0
    max_iou = float(max(support_ious)) if support_ious else 0.0
    std_iou = float(np.std(support_ious)) if support_count > 1 else 0.0
    mean_score = float(np.mean(support_scores)) if support_scores else 0.0
    max_score = float(max(support_scores)) if support_scores else 0.0
    mean_area = float(np.mean(support_areas)) if support_areas else 0.0
    anchor_area = (aw * ah) / max(W * H, 1e-6)

    feat.extend([
        support_count / denom_models,
        sum_iou,
        mean_iou,
        max_iou,
        std_iou,
        ...
        mean_area,
        anchor_area,
        float(anchor_score) * max_iou,
        float(anchor_score) * mean_iou,
    ])
...
def build_training_sets(
    ...
    """
    X_coords, y_coords, w_coords = [], [], []
    X_iou, y_iou, w_iou = [], [], []
    cls_labels, cls_weights = [], []
    ...
    for iid, meta in images_by_id.items():
        ...
        for mi, m in enumerate(models):
            for (b, s) in per_model.get(m, {}).get(iid, []):
                ...
                weight_base = 0.5 + float(s) * 0.5 + float(iou)
                w_iou.append(weight_base)
                cls_labels.append(1 if (gi >= 0 and iou >= label_iou_pos) else 0)
                cls_weights.append(weight_base)
                if gi >= 0 and iou >= label_iou_pos:
                    ...
                    w_coords.append(max(iou, 1e-3))

    X_coords = np.stack(X_coords, axis=0) if len(X_coords) else None
    y_coords = np.stack(y_coords, axis=0) if len(y_coords) else None
    w_coords = np.array(w_coords, dtype=np.float32) if len(w_coords) else None
    X_iou = np.stack(X_iou, axis=0) if len(X_iou) else None
    y_iou = np.array(y_iou, dtype=np.float32) if len(y_iou) else None
    w_iou = np.array(w_iou, dtype=np.float32) if len(w_iou) else None
    y_cls = np.array(cls_labels, dtype=np.int32) if len(cls_labels) else None
    w_cls = np.array(cls_weights, dtype=np.float32) if len(cls_weights) else None

    if w_coords is not None:
        w_coords = np.clip(w_coords, 1e-3, None)
    if w_iou is not None:
        w_iou = np.clip(w_iou, 1e-3, None)
    if w_cls is not None:
        w_cls = np.clip(w_cls, 1e-3, None)
    ...
    X_cls = X_iou if X_iou is not None else None

    return X_coords, y_coords, w_coords, X_iou, y_iou, w_iou, X_cls, y_cls, w_cls
...
def make_cls_classifier(method: str = "xgb"):
    if method == "xgb" and HAVE_XGB:
    ...
    for iid, meta in images_by_id.items():
        ...
        if cls_model is not None:
            if hasattr(cls_model, "predict_proba"):
                cls_scores = cls_model.predict_proba(X)
                cls_scores = np.asarray(cls_scores)
                if cls_scores.ndim == 2:
                    cls_scores = cls_scores[:, -1]
            else:
                cls_scores = cls_model.predict(X)
                cls_scores = np.asarray(cls_scores)
        ...
        base_scores = np.clip(yiou, 0.0, 1.0)
        if np.isscalar(cls_scores):
            scores = base_scores * float(cls_scores)
        else:
            scores = base_scores * cls_scores
...
def main():
    ...
    ap.add_argument("--cls_model", choices=["xgb","rf"], default="xgb", help="Classifier for anchor reweighting")
    args = ap.parse_args()
    ...
    Xc, yc, wc, Xi, yi, wi, Xcls, ycls, wcls = build_training_sets(
    ...
    if Xc is None or yc is None or Xi is None or yi is None or Xcls is None or ycls is None:
    ...
    cls_model = make_cls_classifier(args.cls_model)

    print("\n[FIT] coordinate regressor...")
    if wc is not None:
        coord_reg.fit(Xc, yc, sample_weight=wc)
    else:
        coord_reg.fit(Xc, yc)
    print("[FIT] IoU regressor...")
    if wi is not None:
        iou_reg.fit(Xi, yi, sample_weight=wi)
    else:
        iou_reg.fit(Xi, yi)
    print("[FIT] classification head...")
    if wcls is not None:
        cls_model.fit(Xcls, ycls, sample_weight=wcls)
    else:
        cls_model.fit(Xcls, ycls)
    ...
    try:
        ...
        joblib.dump(cls_model, os.path.join(args.outdir, "cls_model.joblib"))
